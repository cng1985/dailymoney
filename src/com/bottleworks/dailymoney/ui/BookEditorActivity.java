...
public class BookEditorActivity extends ContextsActivity implements android.view.View.OnClickListener{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(Res.layout.bookeditor);
        initIntent();
        initialEditor();
    }
    ...
    private void initIntent() {
        ...
        if(modeCreate){
            setTitle(Res.string.title_bookeditor_create);
        }else{
            setTitle(Res.string.title_bookeditor_update);
        }
    }
    ...
    private void initialEditor() {
        nameEditor = (EditText)findViewById(Res.id.bookeditor_name);
        nameEditor.setText(workingBook.getName());
        
        symbolEditor = (EditText)findViewById(Res.id.bookeditor_symbol);
        symbolEditor.setText(workingBook.getSymbol());
        
      //initial spinner
        positionEditor = (Spinner) findViewById(Res.id.bookeditor_symbol_position);
        ...
        noteEditor = (EditText)findViewById(Res.id.bookeditor_note);
        noteEditor.setText(workingBook.getNote());
        
        okBtn = (Button)findViewById(Res.id.btn_ok);
        if(modeCreate){
        ...
        cancelBtn = (Button)findViewById(Res.id.btn_cancel); 
    ...
    private void doOk(){   
      ...
      //verify
        if(Spinner.INVALID_POSITION==positionEditor.getSelectedItemPosition()){
            GUIs.shortToast(this,i18n.string(Res.string.cmsg_field_empty,i18n.string(Res.string.label_symbol_position)));
            return;
        }
        ...
        if (modeCreate) {
            ...
            GUIs.shortToast(this, i18n.string(Res.string.msg_book_created, name));
        } else {
            idp.updateBook(book.getId(),workingBook);
            GUIs.shortToast(this, i18n.string(Res.string.msg_book_updated, name));
            setResult(RESULT_OK);
            finish();
        }
        setResult(RESULT_OK);
