...
public class DetailListHelper implements OnItemClickListener{
    ...
    private static int[] bindingTo = new int[] { Res.id.detlist_item_layout,Res.id.detlist_item_layout_inner,Res.id.detlist_item_from, Res.id.detlist_item_to, Res.id.detlist_item_money,Res.id.detlist_item_note,Res.id.detlist_item_date };
    ...
    public void setup(ListView listview){
        ...
        case 2:
            layout = Res.layout.detlist_item2;
            break;
        case 3:
            layout = Res.layout.detlist_item3;
            break;
        case 4:
            layout = Res.layout.detlist_item4;
            break;
        default:
            layout = Res.layout.detlist_item1;
        }
    ...
    private Map<String, Object> toDetailMap(Detail det,DateFormat format){
        ...
        String from = fromAcc==null?det.getFrom():(i18n.string(Res.string.label_detlist_from,fromAcc.getName(),AccountType.getDisplay(i18n, fromAcc.getType())));
        String to = toAcc==null?det.getTo():(i18n.string(Res.string.label_detlist_to,toAcc.getName(),AccountType.getDisplay(i18n, toAcc.getType())));
    ...
    class ListViewBinder implements SimpleAdapter.ViewBinder{
        ...
        public boolean setViewValue(View view, Object data, String text) {
            ...
            if("layout".equals(name)){
                ...
                int drawid;
                if( (flag & 1) == 1){//expense
                    drawid = Res.drawable.selector_expense;
                    last = AccountType.EXPENSE;
                }else if( (flag & 2) == 2){//income
                    drawid = Res.drawable.selector_income;
                    last = AccountType.INCOME;
                }else if( (flag & 4) == 4){
                    drawid = Res.drawable.selector_asset;
                    last = AccountType.ASSET;
                }else if( (flag & 8) == 8){
                    drawid = Res.drawable.selector_liability;
                    last = AccountType.LIABILITY;
                }else if( (flag & 16) == 16){
                    drawid = Res.drawable.selector_other;
                    last = AccountType.OTHER;
                }else{
                    drawid = Res.drawable.selector_unknow;
                    last = null;
                }
            ...
            if(AccountType.INCOME.equals(last)){
               ((TextView)view).setTextColor(activity.getResources().getColor(Res.color.income_fg));
//               if("money".equals(name)){
//                   ((TextView)view).setText(text);
//                   return true;
//               }
            }else if(AccountType.ASSET.equals(last)){
                ((TextView)view).setTextColor(activity.getResources().getColor(Res.color.asset_fg)); 
            }else if(AccountType.EXPENSE.equals(last)){
                ((TextView)view).setTextColor(activity.getResources().getColor(Res.color.expense_fg));
//                if("money".equals(name)){
//                    ((TextView)view).setText(text);
//                    return true;
//                }
            }else if(AccountType.LIABILITY.equals(last)){
                ((TextView)view).setTextColor(activity.getResources().getColor(Res.color.liability_fg)); 
            }else if(AccountType.OTHER.equals(last)){
                ((TextView)view).setTextColor(activity.getResources().getColor(Res.color.other_fg)); 
            }else{
                ((TextView)view).setTextColor(activity.getResources().getColor(Res.color.unknow_fg));
            }
            ...
            if("from".equals(name)){
                ...
                if(last!=lastFrom){
                    if(AccountType.INCOME== lastFrom){
                        view.setBackgroundDrawable(activity.getResources().getDrawable(Res.drawable.selector_income));
                    }else if(AccountType.EXPENSE== lastFrom){
                        view.setBackgroundDrawable(activity.getResources().getDrawable(Res.drawable.selector_expense));
                    }else if(AccountType.ASSET== lastFrom){
                        view.setBackgroundDrawable(activity.getResources().getDrawable(Res.drawable.selector_asset));
                    }else if(AccountType.LIABILITY== lastFrom){
                        view.setBackgroundDrawable(activity.getResources().getDrawable(Res.drawable.selector_liability));
                    }else if(AccountType.OTHER== lastFrom){
                        view.setBackgroundDrawable(activity.getResources().getDrawable(Res.drawable.selector_other));
                    }else{
                        view.setBackgroundDrawable(activity.getResources().getDrawable(Res.drawable.selector_unknow));
                    }
                }
            }
